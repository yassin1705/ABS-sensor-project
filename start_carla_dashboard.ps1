...
if ($listener) {
    try {
        $health = Invoke-RestMethod -Uri "$apiBase/api/health" -TimeoutSec 3
        if ($health.live_api_version -ne 3) { throw "Incompatible API version" }
    } catch {
        throw "Port $apiPort is occupied by an outdated or incompatible process (PID $($listener.OwningProcess))."
    }
} else {
